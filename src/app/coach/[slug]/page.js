    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-6">This coach page doesn't exist or has been removed.</p>
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center gap-4">
            {user ? (
              <>
                ...
                <Link href={user.role === 'coach' ? '/dashboard' : '/user/dashboard'} className="text-sm text-gray-600 hover:text-gray-900">
                  Dashboard
                </Link>
              </>
            ) : (
              <Link href={`/login?coach=${slug}`} className="text-sm text-gray-600 hover:text-gray-900">
                Sign In
              </Link>
            )}
          </div>
      ...
      <main className="container py-16">
        <div className="max-w-2xl mx-auto text-center">
          <div className="w-24 h-24 rounded-full mx-auto mb-6 flex items-center justify-center text-3xl bg-blue-100 text-blue-600">
            {coach.logo_url ? (
              <img src={coach.logo_url} alt={coach.business_name} className="w-full h-full rounded-full object-cover" />
            ) : (
          ...
          <p className="text-xl text-gray-600 mb-8">
            {coach.bio || `Welcome to ${coach.business_name}. Subscribe to access exclusive content.`}
          </p>
          ...
          <div className="p-8 rounded-xl bg-white border border-gray-200 mb-8">
            ...
            {!isActive ? (
              ...
              >
                {checkoutLoading ? 'Loading...' : user ? 'Subscribe Now' : 'Sign Up & Subscribe'}
              </button>
            )}
          </div>
          ...
          <div className="p-12 rounded-xl bg-white border border-gray-200 border-dashed">
            ...
            <p className="text-gray-600">Subscribe to unlock exclusive content from {coach.business_name}</p>
          </div>
        </div>
      </main>
      ...
      <footer className="border-t border-gray-200 py-6">
        <div className="container text-center text-sm text-gray-600">
          Powered by <Link href="/" className="link">Daily Companion</Link>
        </div>
      </footer>
    </div>
  );

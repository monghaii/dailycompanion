...
const ENCRYPTION_KEY = process.env.KIT_ENCRYPTION_KEY || 'your-32-character-secret-key!!!'; // Must be 32 characters
...
export async function POST(request) {
  try {
    const { sessionToken, kitApiKey, kitEnabled, kitFormId, kitTags } = await request.json();
...
export async function GET(request) {
  try {
    ...
    const { data: coach } = await supabase
      .from('coaches')
      .select('kit_enabled, kit_form_id, kit_tags, kit_last_sync, kit_sync_status, kit_error_message')
      .eq('profile_id', session.user_id)
      .single();
